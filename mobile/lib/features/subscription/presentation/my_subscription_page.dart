...
class MySubscriptionPage extends ConsumerWidget {
  ...
  Widget _bigTierCard(AppLocalizations l, GpColors gp, SubscriptionState sub,
    ...
    // `total == 0` only — every tier shares the same monthly cap
    // per the backend seed (gym network is the only differentiator),
    // so the only "no denominator" case is the brief window before
    // the subscription has hydrated. Earlier `total <= 0` guarded
    // against a Diamond `-1` sentinel that no longer exists.
