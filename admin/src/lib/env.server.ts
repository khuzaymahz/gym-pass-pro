...
// FastAPI for the admin-exchange envelope) plus server-side URLs
// like `API_BASE_URL` (`http://backend:8000` inside docker — not
// reachable from the browser). The `import "server-only"` directive
// at the top of this file makes Next.js refuse to bundle it into any
// Client Component; if an accidental import shows up, the build
// fails with a clear message instead of crashing in the browser.
//
...
// Dev defaults live in `.env.example` and `docker-compose.yml`, NOT
// here.
//
// Pattern mirrors `gym-partner/src/lib/env.server.ts` for consistency
// across both Next.js apps.
