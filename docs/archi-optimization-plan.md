...
> **v6 revision — intent-named policy flags.** Adding `staging` to the `Literal` is correct, but `is_dev` is also referenced by ~12 scattered call sites today (mock SMS, mock payments, demo seed gates, debug toggles, frictionless OTP, CORS wildcarding). Carving a single `staging` branch in `validate_production_safety()` leaves all of them silently still keyed on `is_dev` — so by default `staging` behaves like production *except* for the secrets check, which is the opposite of the goal (we want `staging` to *act* like production but with mocks still in place).
>
> Recommendation: introduce intent-named properties on the settings object and migrate the call sites once. After that, the `staging` branch in `validate_production_safety()` becomes a single policy decision per intent, not a code search.
>
> ```python
> @property
> def should_mock_sms(self) -> bool:
>     return self.sms_provider == "mock"          # explicit, not env-derived
>
> @property
> def should_mock_payments(self) -> bool:
>     return self.payments_provider == "mock"
>
> @property
> def should_seed_demo_data(self) -> bool:
>     return self.app_env == "development"        # never staging or prod
>
> @property
...
> ```
>
> The provider strings (`sms_provider`, `payments_provider`) become orthogonal knobs: `staging` defaults to `mock` for both, but a future "real-SMS staging dry-run" doesn't need a code change — only an env-var flip. This collapses the staging/prod environment matrix from "every-flag-per-env" to "policy-per-intent", which is the only sustainable shape as the env count grows past three.
>
> Acceptance: `grep -n 'is_dev' backend/app/` returns zero hits outside `config.py` after this lands.
...
### 1.6 Operational gaps the plan previously omitted (v6 addition)

The original plan covered code-level staging readiness but skipped operational concerns that bite once staging is the daily-loop env:

1. **`audit_log` retention.** Every domain mutation writes one row in the same transaction (correct). At even moderate traffic — 100 check-ins/day across 10 gyms — that table grows by ~3k rows/month. After a year, it's the largest table by row count, and admin "recent activity" queries start scanning a multi-million-row table. **Recommendation:** partition `audit_log` by month (Postgres native declarative partitioning) and add a Celery beat task in `workers/tasks/scheduled.py` that drops partitions older than 12 months. Phase B work, but design the partition layout in Phase A so the cutover is non-blocking.

2. **Database backups.** Pre-prod runs Postgres in a container with the data dir volume-mounted on the VM. There is no backup script in the repo. A disk failure or a stray `truncate` loses every member registration to date. **Recommendation:** Phase A adds `scripts/db-backup.sh` (logical: `pg_dump | gzip > /backups/YYYY-MM-DD.sql.gz`, kept 7 days local + uploaded to an off-host bucket if `BACKUP_BUCKET` is set) and a cron entry on the VM. Phase C upgrades to wal-g or pgBackRest if the staging window proves the logical-dump approach too slow.

3. **Realtime WS auth expiry handling.** §3.4 says "keep WS where it is." That's right structurally, but the existing client backoff doesn't distinguish auth-expired from network-error. After 1 hour idle, the access token expires, the WS server closes the connection, and the client reconnects... using the same expired token, ad infinitum, until the next foregrounding triggers an HTTP call that refreshes the bearer. **Recommendation:** add a typed close-frame from the server (`{"type": "auth_expired"}`); the client triggers a token refresh via the existing `core/api/api_client.dart` coalescing path *before* the next reconnect attempt.

These are not Phase A blockers (staging will work without them) but they belong in the Phase B / C backlog explicitly rather than waiting to be re-discovered in incident review.
...
| 2.10 | ~~Mobile~~ | ~~Migrate hand-rolled `fromJson` to `freezed` + `json_serializable` once `packages/api-client-dart` lands.~~ **(v6: moved to Phase B — this is a refactor, not a production-quality blocker. Hand-rolled `fromJson` with unsafe casts doesn't prevent prod from running; it makes it brittle. Belongs with the codegen migration in §6.2 step 10.)** | Appendix D §3 |
...
Extract `design/project/tokens.json`. A small Node script (`scripts/codegen-tokens.mjs`) reads it and emits the four downstream files; **the outputs are committed to git** and a CI job re-runs the script + `git diff --exit-code` to fail any PR where someone hand-edited a generated file or forgot to regenerate after a tokens.json change.

> **v6 revision — why not generate-on-pub-get.** The original "generate the Dart file on `flutter pub get`" approach binds the build to a Node script that must run before every checkout works. That couples Flutter-side tooling to Node-side tooling (slow, brittle, breaks IDE auto-build, breaks `flutter test` in CI runners that don't have Node). Commit-the-output is the same pattern OpenAPI codegen uses below (§3.1) and the same pattern protobuf generation uses everywhere. The CI check catches drift; the local build stays fast.
>
> Concretely:
> ```bash
> # scripts/codegen-tokens.mjs (runs locally, output checked into git)
> node scripts/codegen-tokens.mjs
> # emits:
> #   packages/tokens/tailwind.preset.js
> #   mobile/lib/core/theme/gp_tokens.dart
> #   design/project/colors_and_type.css        (regenerated from JSON, no longer source-of-truth)
> # CI runs the same script + `git diff --exit-code` on those three paths.
> ```
...
Enforcement: written policy in CODEOWNERS + this doc, **PR-time grep check** in CI (`scripts/lint-mobile-imports.sh` — fails if `features/x/` imports `features/y/`). Originally proposed: `dart_code_metrics` with a custom `avoid-cross-feature-imports` rule.

> **v6 revision — right-sized enforcement.** For a solo developer, a custom `dart_code_metrics` rule is months of tool maintenance for a constraint that's currently always satisfied (Appendix D confirms zero cross-module presentation imports). A 20-line bash script that runs in CI catches the same regression with no plugin ecosystem to maintain. Backend's `import-linter` stays — it's a single-config tool with native Python support and the dependency-direction risk is higher there because services / repositories / models are easier to accidentally cross. Promote the dart rule to a `dart_code_metrics` plugin only if/when a second mobile developer joins and starts cross-importing.
...
> **v6 revision — reordered + workspace conversion deferred.** The original ordering put the pnpm workspace + folder move (steps 3–4) before the codegen + extraction work. That's the highest-risk lowest-value path: the workspace conversion is a multi-day tooling adventure (every CI workflow, every Dockerfile `WORKDIR`, every nginx upstream, every npm script gets touched) that **does not unblock any feature work**. Codegen, in contrast, eliminates 40+ DTO maintenance hits the next time the OpenAPI schema changes — that's the high-leverage move.
>
> New order: extract the **logical** packages first (using top-level folders like `shared/tokens/`, `shared/api-client-ts/`), prove the codegen + token pipeline works, *then* do the workspace conversion as a final cosmetic move (mostly `git mv` + import-path rewrite). If the workspace conversion turns out to be more pain than gain for a solo project, step 11 is just skipped — the package extractions still stand on their own as top-level folders.

1. **Delete root cruft** — `dump.err`, `gympass-data.sql`, vestigial Vite host (root `package.json` + `vite.config.js` + `index.html`), legacy `Gym pass Mobile App/`. Run `git log --since=6months -- "Gym pass Mobile App/"` first to confirm nothing recent depends on it.
2. **Bootstrap `.github/workflows/ci.yml`** — lint+test for all four code surfaces + the v6-introduced `scripts/codegen-tokens.mjs` drift check + `scripts/lint-mobile-imports.sh`. **No deploy steps.** Just guardrails.
3. **Extract `shared/tokens/`** (§3.5). Top-level folder, not a workspace package yet. `tokens.json` + `codegen-tokens.mjs` script + committed outputs + CI drift check.
4. **Generate `shared/api-client-ts/`** (orval) from `/api/openapi.json`. **Dual-source migration:** commit the generated types as `*.gen.ts` and migrate admin + partner SDKs file-by-file, removing hand-typed DTOs as their callers move. Resolves the `CheckinStatus.gender_locked` drift (§3.3) by removing the hand-typed enum. Folds in row #6 of §5.
5. **Extract `shared/ui/`** starting with `PendingButton.tsx`. Migration order in §3.2. Folds in rows #3, #9 of §5 (extract `usePhotoUpload`, `useAmenitiesState`).
6. **Extract `shared/auth-client/`** — HMAC envelope, exchange coalescing, `SESSION_EXPIRED` redirect.
7. **Generate `shared/api-client-dart/`** (openapi-generator dart-dio **or** hand-curated `freezed` schemas — decide based on a 1-day spike of the openapi-generator dart-dio output quality). Migrate mobile DTOs feature-by-feature. Folds in row #5 of §5.
8. **Mobile large-page refactor (rows #1, #2 of §5).** Specifically: `ExplorePageState` watching 6 providers is a state-shape problem, not a layout problem — introduce `ExploreController` (Riverpod `Notifier`) that owns the combined view-model so `ExplorePage` subscribes to *one* provider. Same controller pattern for `SettingsPage` and `PlansPage`. Sub-widget extraction is a secondary consequence, not the primary fix. **v6 amendment to Appendix D.6.**
9. **`audit_log` monthly partitioning** (§1.6 v6 addition) — declarative partition layout + Celery retention task. Time-bounded.
10. **Wire dependency-rule enforcement** (§3.7): `.importlinter` for backend (high yield, single tool); `eslint-plugin-import/no-restricted-paths` for TS; mobile relies on the bash check from §3.7 unless a second mobile developer joins. CI fails on violations.
11. **Backend contract tests** in `apps/backend/tests/contract/` using schemathesis. Locks down the OpenAPI surface feeding the generated clients.
12. **(Optional, last) Convert root to pnpm workspace + Turborepo + move folders to `apps/` + `packages/` + `infra/`** per §6.0. One PR, mechanical. **Skip if the cost outweighs the benefit for a one-developer project** — the package extractions in steps 3–7 stand on their own at top-level folders.
...
> **v6 revision — the fix is controllers, not sub-widget extraction.** Both Appendix D.6.1 and §5 row #5 frame `ExplorePageState`-watches-6-providers as a layout problem, with "split into sub-widgets" as the prescribed fix. That misdiagnoses the root cause: even after splitting the page into a dozen `ConsumerWidget`s, *each* of those subwidgets still needs to watch the same providers and reason about their combined state, because the view-model is genuinely composite (filter chips + map + list + selected-gym overlay all derive from the same filtered gym set). The right shape is a `Riverpod Notifier` (`ExploreController`) that exposes a single `ExploreViewModel` provider. The page subscribes to one thing; sub-widgets become trivial. Same prescription applies to `SettingsPage` (a `SettingsController` orchestrating the dozen toggles) and `PlansPage` (a `PlansController` owning tier-grid + duration-toggle + promo state).
>
> Net effect on Phase B §6.2 step 8: the refactor is one PR per page that introduces a controller + collapses the page body, not a multi-PR sub-widget extraction effort. Smaller blast radius, larger conceptual win.
...
- 2026-05-16 v6 — **Second-pass revisions after one round of pre-prod field use.** Concrete changes:
   ...
   4. **§3.5 (design tokens)** — switched from generate-on-pub-get to committed outputs + CI drift check. Decouples Flutter build from Node tooling.
   5. **§3.7 (dependency rules)** — scoped back mobile enforcement from a custom `dart_code_metrics` plugin to a 20-line bash check in CI; promote later if a second developer joins. Backend `import-linter` kept as-is (highest ROI single tool).
   6. **§6.2 (Phase B order)** — codegen + extractions first using top-level `shared/` folders; pnpm workspace + `apps/` move moved to step 12 and marked optional. The high-leverage moves no longer gate on the high-risk tooling move.
   7. **§6.1 Step 3 (mkcert)** — added explicit per-platform CA-trust recipe (macOS / Linux / Windows / Android emulator / iOS Simulator / real Android device) committed to `infra/scripts/mkcert.sh`.
   8. **Appendix D.6 + §6.2 step 8** — re-diagnosed `ExplorePageState`-watches-6-providers as a state-shape problem fixed by a `Notifier`-based controller, not a sub-widget extraction problem. Same prescription for `SettingsPage` + `PlansPage`. Smaller blast radius per PR.
