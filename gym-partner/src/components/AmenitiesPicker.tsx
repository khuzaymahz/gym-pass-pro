...
/// Preset list keeps the picker scannable for the 80% case (a partner
/// is checking off the obvious things their gym has) while the custom
/// input below handles the long tail. Grouped into "basics" — what
/// every gym is expected to answer — and "popular" — common but
/// optional. Order within a group is roughly weighted by how often
/// members ask for the amenity in the explore filter.
const PRESETS: { value: string; group: "basics" | "popular" }[] = [
  // Basics — table-stakes amenities. Most gyms tick most of these.
  ...
  // Popular — present at well-equipped gyms, drives the tier signal.
...
export function AmenitiesPicker({
  ...
  // Anything not in the preset list is treated as a custom entry —
  // shown as a removable chip below the preset grid so partners can
  // see and clean up legacy amenities they typed in the old free-form
  // text field.
  ...
  function addCustom(): void {
    // Lowercase + trim so "WiFi" and "wifi " don't both end up in
    // the list. Strip commas so a partner pasting "yoga, pilates"
    // doesn't smuggle in two values that the backend then rejects
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-4">
        {items.map(({ value }) => {
          ...
          // Disable an unchecked tile when we've hit the cap — a
          // checked tile must remain enabled so the partner can
          // uncheck and free up a slot.
